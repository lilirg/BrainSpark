...
│   │   ├── report.py                        # 报告请求/响应
│   │   ├── knowledge.py                     # 知识库请求/响应
│   │   ├── training.py                      # 训练计划请求/响应
│   │   └── common.py                        # 通用响应格式
│   ├── services/                            # 业务逻辑
│   │   ├── __init__.py
│   │   ├── cognitive_analyzer.py            # 认知分析服务
│   │   ├── report_generator.py              # 报告生成服务
│   │   ├── training_recommender.py          # 训练推荐服务
│   │   ├── llm_service.py                   # LLM 服务封装
│   │   ├── embedding_service.py             # 向量化服务
│   │   └── compliance_service.py            # 合规服务（脱敏）
│   ├── rag/                                 # RAG 模块
│   │   ├── __init__.py
...
│   │   ├── assessment_prompt.py             # 分析提示词
│   │   ├── training_prompt.py               # 训练推荐提示词
│   │   └── safety_prompt.py                 # 安全过滤提示词
│   ├── utils/                               # 工具函数
│   │   ├── __init__.py
│   │   ├── llm_utils.py                     # LLM 工具
...
class MultiPathRetriever:
    ...
    async def search(self, query: str, k: int = 5,
        ...
        """
        # 1. 向量检索
        vector_index = self.collection.index()
...
class CrossEncoderReranker:
    ...
    def rerank(self, query: str, documents: List[Dict[str, Any]],
        ...
        for doc, score in zip(documents, scores):
            ...
            doc["final_score"] = doc.get("score", 0) * 0.6 + float(score) * 0.4
...
|--------|----------|-------------|------|
| `POST` | `/api/v1/assessments/analyze` | 认知分析 | JWT |
| `POST` | `/api/v1/reports/generate` | 生成报告 | JWT |
| `POST` | `/api/v1/reports/{id}/feedback` | 提交报告反馈 | JWT |
| `POST` | `/api/v1/training/plans` | 生成训练计划 | JWT |
| `POST` | `/api/v1/knowledge/index` | 索引知识 | Admin |
| `POST` | `/api/v1/knowledge/search` | 搜索知识 | 公开 |
| `GET` | `/api/v1/knowledge/categories` | 知识分类列表 | 公开 |
| `GET` | `/health` | 健康检查 | 无 |
...
class Settings(BaseSettings):
    ...
    MILVUS_COLLECTION: str = "brainspark_knowledge"
